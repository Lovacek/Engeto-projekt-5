...
def vytvoreni_tabulky(): # Funkce pro vytvoření zakladní tabulky (pokud již neexistuje), se kterou bude program dále pracovat. 
    try:
        cursor.execute('''
...
def pridat_ukol(): #Funkce přidávající nový úkol do databázové tabulky. Uživatel zadává nazev a popis úkolu, který chce přidat. V případě chybného zadání je funkcí upozorněn.
    while True:
...
def odstranit_ukol(): #Funkce pro odstranění úkolu ze seznamu. Funkce vzobrazí seznam úkolu, kde uživatel zadá ID úkolu, který chce odstranit. 
        while True:
            try:
                seznam_ukolu_del = []
                seznam_ID_del = []
...
def aktualizovat_ukol(): #Funkce mění stav vybraného úkolu na "Hotovo" nebo "Probíhající"
    while True:
        try:
            seznam_ukolu_act = []
            seznam_ID_act = []
